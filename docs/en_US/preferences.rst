...
* When the *Confirm on close or refresh?* switch is set to *True*, pgAdmin will
  ...
  them to continue. It is *False* by default, so pgAdmin closes without
  asking.

* When the *Double-click opens the Query Tool?* switch is set to *True*,
  double-clicking an object in the Object Explorer opens the Query Tool on it.
  For a table or view, a ``SELECT`` limited to *Double-click row limit* rows is
  generated and executed; for any other object, the object's ``CREATE`` script
  is loaded into the editor without being executed. Collection nodes, and
  objects which have no ``CREATE`` script, continue to expand and collapse on
  double-click as before. A Query Tool tab opened this way is reused by the
  next double-click in the same database; tabs you opened yourself are never
  reused.

* Use the *Double-click row limit* field to set the number of rows the
  ``SELECT`` generated by double-clicking a table or view is limited to. It
  only applies when *Double-click opens the Query Tool?* is set to *True*.
...
* When the *Prompt to commit/rollback active transactions?* switch is set to
  *True*, the editor will prompt the user to commit or rollback changes when
  exiting the Query Tool while the current transaction is not committed. It is
  *False* by default, so an open transaction is rolled back, as the connection
...
* When the *Prompt to save unsaved data changes?* switch is set to *True*, the
  ...
  editor. It is *False* by default, so uncommitted grid edits are discarded
  without asking.
...
* When the *Prompt to save unsaved query changes?* switch is set to *True*, the
  ...
  the Query Tool. It is *False* by default, so unsaved query text is discarded
  without asking.
...
* When the *Execute script runs the query at the cursor?* switch is set to
  *True*, the *Execute script* button runs only the query the cursor sits in,
  delimited by blank lines, when nothing is selected. Set it to *False* to have
  *Execute script* run the whole contents of the editor. Either way, a
  highlighted selection is always executed as-is, and the *Execute query*
