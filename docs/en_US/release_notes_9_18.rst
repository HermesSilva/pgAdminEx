...
************
  ...
  | Show a ``Tables`` shortcut as the first child of every database in the Object Explorer, listing the tables of the ``public`` schema so the common case no longer requires descending through Schemas > public > Tables. The entries are ordinary table nodes, so the context menu, properties, DDL and View/Edit Data behave exactly as they do under a schema. The shortcut is omitted for a database with no accessible ``public`` schema.
  | Stop asking for confirmation when a tab or the application is closed, by defaulting ``prompt_save_query_changes``, ``prompt_save_data_changes``, ``prompt_commit_transaction`` and ``confirm_on_refresh_close`` to False. Closing a Query Tool tab now discards unsaved query text and uncommitted grid edits, and rolls back an open transaction, without interrupting; closing pgAdmin no longer prompts either. Each preference can be switched back on individually.
  | Open the Query Tool by double-clicking an object in the Object Explorer, in the manner of DBeaver: a table or view gets a ``SELECT`` limited to 1000 rows, generated and executed straight away, while any other object gets its ``CREATE`` script loaded without being run. Collection nodes keep expanding and collapsing as before. The tab opened this way is reused by the next double-click in the same database, so browsing does not pile up tabs; tabs opened by hand are never reused. Controlled by the new ``dbl_click_opens_query_tool`` and ``dbl_click_row_limit`` browser preferences.
  | Make *Execute script* run only the query the cursor is positioned in, delimited by blank lines, when nothing is selected - in the manner of DBeaver - so a script of several statements can be run one at a time without highlighting each. A highlighted selection is still executed as-is, and *Execute query* is unchanged. The new ``execute_script_at_cursor`` preference, on by default, restores the previous whole-editor behaviour when set to False.
