...
def register_browser_preferences(self):
    ...
    self.preference.register(
        ...
        help_str=gettext(
            ...
            'is intended before proceeding. Disabled by default: pgAdmin '
    ...
    self.preference.register(
        'display', 'dbl_click_opens_query_tool',
        gettext("Double-click opens the Query Tool?"), 'boolean', True,
        category_label=PREF_LABEL_DISPLAY,
        help_str=gettext(
            'If enabled, double-clicking an object in the Object Explorer '
            'opens the Query Tool, in the manner of DBeaver: for a table or '
            'view it runs a SELECT limited to the number of rows set below, '
            'and for any other object it loads the object\'s CREATE script '
            'without running it. Collection nodes and objects that cannot be '
            'scripted keep expanding and collapsing as usual.'
        )
    )

    self.preference.register(
        'display', 'dbl_click_row_limit',
        gettext("Double-click row limit"), 'integer', 1000,
        min_val=1, category_label=PREF_LABEL_DISPLAY,
        help_str=gettext(
            'The number of rows the SELECT generated by double-clicking a '
            'table or view is limited to. Only applies when '
            '"Double-click opens the Query Tool?" is enabled.'
        )
    )
